// Base Data URL
let baseDataURL = window.location.pathname
const hostname = window.location.hostname

if (/localhost/g.test(hostname)) {
  baseDataURL = '/'
} else if (/github/g.test(hostname)) {
  baseDataURL = '/pakistan_web_dev/'
}

console.log(`--base: ${baseDataURL}`)
...
// tree viewer
let geoFilterClimateRisks
let filterRiskCommidities
let filterRiskHazards
...
async function click_tab (tab_selected) {
  ...
  if (tab === 'intro') {
    ...
    if (!impactsData) {
      ...
      // Show all levels
      geoFilterClimateRisks = new GeographicFilter(impactsData, 'tree')
      filterRiskCommidities = new SimpleFilter({ data: impactsData, id: 'cbo_impact_crop', rootText: 'Commodities', key: 'crop_livestock' })
      filterRiskHazards = new SimpleFilter({ data: impactsData, id: 'cbo_impact_hazard', rootText: 'Hazards', key: 'hazard' })
    }
...
async function renderClimateImpacts (level = 'dist') {
  ...
  // TO-DO: Display items from simpleFilter
  /*
...
function onCheck () {
  ...
  const checkedCount = $('#treeview').swidget('TreeView').element.find('li').filter(function () {
    return $('#treeview').swidget('TreeView').checked($(this))
  }).length
  console.log(checkedCount + ' items checked')
}

function fill_shieldtree () {
  ...
  const data = [{
    ...
    children: [{
      ...
      children: [
        ...
        { id: 'district 2', text: 'District 2' },
        { id: 'district 3', text: 'District 3' }
      ]
    }, {
      id: 'punjab',
      text: 'Punjab'
